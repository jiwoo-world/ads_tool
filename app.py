...
def _ad_copy_normalize_cta(cta: str) -> str:
    """CTA 표시용: 느낌표 제거 후 공백 포함 최대 글자 수까지."""
    s = _ad_copy_strip_cta_trailing_bang(cta)
    if len(s) > _AD_COPY_CTA_MAX_LEN:
        return s[:_AD_COPY_CTA_MAX_LEN]
    return s


def _ad_copy_blocks_from_json(raw: dict[str, Any]) -> list[tuple[int, str, str, str]]:
    ...
    for idx in range(4):
        ...
        cta = _ad_copy_normalize_cta((block.get("cta") or "").strip())
...
def _format_ad_copies_from_json(raw: dict[str, Any]) -> str:
    """
    JSON 응답을 평문으로 변환(재시도·빈 값 판별용).
    - 각 블록은 5줄: `1. 제목` / 본문 1줄 / 본문 1줄 / 본문 1줄 / CTA.
    """
    parts: list[str] = []
    for num, title, body_fmt, cta in _ad_copy_blocks_from_json(raw):
        parts.append(f"{num}. {title}\n{body_fmt}\n{cta}")
    return "\n\n".join(parts)


def _format_ad_copies_html(raw: dict[str, Any]) -> str:
...
def _ad_copy_raw_to_card_blocks(raw: dict[str, Any]) -> list[dict[str, str]]:
    """카드 UI용: 제목·본문(줄바꿈 유지)·CTA."""
    blocks: list[dict[str, str]] = []
    for _num, title, body_fmt, cta in _ad_copy_blocks_from_json(raw):
...
"""
    ...
    raw = run_once()
    text = _format_ad_copies_from_json(raw)
    ...
    return text, _ad_copy_raw_to_card_blocks(raw)
